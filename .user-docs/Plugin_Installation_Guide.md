...
| Archive | `plugins/harness-kit-0.4.1.zip` |
...
> `0.4.1` CLI 설치 smoke는 Codex CLI와 Claude Code 양쪽에서 통과했다. 다만 이는
...
4. local marketplace가 앱에 보이지 않는 버전이면 앱과 같은 사용자 프로필의
   ...
   연다.
5. 플러그인 화면 또는 `/plugins`에서 `harness-kit` `0.4.1`이
   설치·활성 상태인지 확인한다.
...
`0.4.1` CLI 설치 smoke 결과: `install-smoke-verified`. 앱 설치와 직접 모델 호출은
`manual-required`다.
