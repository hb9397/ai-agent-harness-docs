...
아래 항목은 **앱 context에 넣지 않는다**. instruction 파일로 넘긴다.
...
사실 목록으로서 앱 context에 실어도 되고, `api-instruction.md` / `comm-instruction.md`에
실어도 된다. 본 스킬은 **규약(규칙)은 instruction에, 팩트(목록)만 앱 context 가능**을 기본으로 한다.
