...
| 위치 | 상태 | 설명 |
|------|------|------|
| [20260803/260803-2.skill-portfolio-upstream-refresh-impl-maintenance.md](./20260803/260803-2.skill-portfolio-upstream-refresh-impl-maintenance.md) | Track A Phase 3 완료 / Phase 4 실행 승인 대기 | G1로 UI/UX Pro Max `v2.13.0`을 선택하고 exact source/runtime candidate와 3-way diff를 구성했다. MIT LICENSE raw hash는 accepted provenance와 일치하고, script/test 정적 심사와 runtime preview 36개 test가 통과했다. 후보는 `accept-for-staging`이지만 보호 script/test 반입에는 G4 명시 승인이 필요하다. Track B의 `.claude`·`.codex` 독립 hook, 외부 산출물 의미·형식 정규화, 사용자 19 + 관리자 3 계약은 Track A 이후 구현한다. |
