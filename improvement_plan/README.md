...
| 위치 | 상태 | 설명 |
|------|------|------|
| [20260803/260803-2.skill-portfolio-upstream-refresh-impl-maintenance.md](./20260803/260803-2.skill-portfolio-upstream-refresh-impl-maintenance.md) | Track A Phase 9 완료 / Track B Phase B5 완료 / Phase B6 실행 승인 대기 | `harness-kit` `0.3.1` 후보의 Track A는 `main`에 push됐고 `not-release-ready` 수동 gate를 유지한다. Track B는 dual-host baseline(B0), portable bundle(B1), setup lifecycle(B2), bootstrap/context-doc의 의미·형식·외부 inbox routing 정합(B3), dual-host write guard(B4), 16-case portable routing matrix·승인형 normalization(B5)을 고정했다. 실제 host install·trust와 B6 plugin 재생성은 수행하지 않았다. |
