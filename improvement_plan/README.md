...
| 위치 | 상태 | 설명 |
|------|------|------|
| [20260803/260803-2.skill-portfolio-upstream-refresh-impl-maintenance.md](./20260803/260803-2.skill-portfolio-upstream-refresh-impl-maintenance.md) | Track A Phase 8 완료 / Phase 9 실행 승인 대기 | `harness-kit` `0.3.1` 후보의 registry, 14개 canonical skill runner, plugin/projection/freeze, 결정적 build, 격리 Codex·Claude CLI 설치 smoke와 release regression을 통과했다. CLI smoke는 설치·캐시 증적일 뿐 모델 호출이 아니며 Codex·Claude CLI·앱 네 인터페이스의 수동 증적이 없어 상태는 계속 `not-release-ready`다. 다음 Phase는 실행 결과 문서화·최종 diff/commit 경계 검토이며, push/tag/release는 G8 별도 승인 전까지 하지 않는다. Track B의 `.claude`·`.codex` 독립 hook, 외부 산출물 의미·형식 정규화, 사용자 19 + 관리자 3 계약은 Track A 이후 구현한다. |
| [20260803/260803-1.harness-kit-plugin-rename-and-example-removal-impl-doc.md](./20260803/260803-1.harness-kit-plugin-rename-and-example-removal-impl-doc.md) | Phase 8 완료 / release gate 차단 | plugin ID를 `harness-kit`, marketplace를 `hb9397`로 전환하고, root `example/`을 외부 저장소 없이 제거하며 산출물 routing·impl downstream 계약을 정립하는 impl-doc 계획; 앱·모델 증적과 publish 별도 승인은 후속 조건 |
...
| [20260729/플러그인 전환 및 스킬 거버넌스 리팩토링 작업 계획서.md](./20260729/플러그인%20전환%20및%20스킬%20거버넌스%20리팩토링%20작업%20계획서.md) | Phase 0~10 구현 완료 / 직접 인터페이스 검증 대기 | 사용자 플러그인 전환, 관리자/사용자 스킬 분리, upstream governance, 문서 최신화, release gate 구현 완료. Codex CLI·Claude Code CLI의 로컬 marketplace 설치 smoke는 통과했고 Codex·Claude CLI/앱 네 인터페이스의 실제 모델 호출 증적과 관리자 최종 승인이 남아 있음 |
...
- 사용자 프로젝트는 `harness-kit@hb9397` 플러그인을 설치한 뒤 `harness-setup`을 실행한다.
...
- 현재 릴리스 후보는 자동 회귀 검증과 Codex CLI·Claude Code CLI의 격리 설치
  smoke를 통과했다. Codex 앱·Claude 앱의 직접 증적이 없어
