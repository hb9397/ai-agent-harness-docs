...
| 위치 | 상태 | 설명 |
|------|------|------|
| [20260803/260803-2.skill-portfolio-upstream-refresh-impl-maintenance.md](./20260803/260803-2.skill-portfolio-upstream-refresh-impl-maintenance.md) | Track A Phase 7 완료 / Phase 8 실행 승인 대기 | UI/UX Pro Max `v2.13.0`을 반영한 `harness-kit` `0.3.1` 후보 archive를 Codex·Claude 각각 19 skills / 0 agents로 재생성했다. source/projection/plugin 및 UI/UX runtime 45개 파일 hash가 일치하고, 기존 `0.3.0` archive hash는 보존됐다. 다음 Phase는 전체 runner·registry·설치 표면의 release gate 재검증이며, push/tag/release는 아직 하지 않는다. Track B의 `.claude`·`.codex` 독립 hook, 외부 산출물 의미·형식 정규화, 사용자 19 + 관리자 3 계약은 Track A 이후 구현한다. |
