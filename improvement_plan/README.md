...
| 위치 | 상태 | 설명 |
|------|------|------|
| [20260803/260803-2.skill-portfolio-upstream-refresh-impl-maintenance.md](./20260803/260803-2.skill-portfolio-upstream-refresh-impl-maintenance.md) | Track A Phase 9 완료 / Track B 구현 대기 | `harness-kit` `0.3.1` 후보의 upstream refresh, canonical 반영, plugin 재생성, 14개 runner, 격리 Codex·Claude CLI 설치 smoke와 release regression을 완료하고 `main`에 push했다. tag/GitHub Release는 생성하지 않았으며, 모델 호출이 아닌 CLI smoke만 존재하므로 상태는 `not-release-ready`다. Track B의 `.claude`·`.codex` 독립 hook, 외부 산출물 의미·형식 정규화, 사용자 19 + 관리자 3 계약은 별도 구현·승인과 최소 `0.4.0` 결정이 필요하다. |
