...
| 위치 | 상태 | 설명 |
|------|------|------|
| [20260803/260803-2.skill-portfolio-upstream-refresh-impl-maintenance.md](./20260803/260803-2.skill-portfolio-upstream-refresh-impl-maintenance.md) | Track A Phase 9 완료 / Track B Phase B1 완료 / Phase B2 실행 승인 대기 | `harness-kit` `0.3.1` 후보의 Track A는 `main`에 push됐고 `not-release-ready` 수동 gate를 유지한다. Track B는 read-only 사례 topology와 dual-host capability baseline(B0), plugin-neutral portable bundle schema·template·clean-fixture `-Plan` 검증(B1)을 고정했다. B1은 host-local 설치나 trust를 수행하지 않았고, 다음 B2는 setup 초기·갱신·manual adoption 흐름을 구현한다. |
