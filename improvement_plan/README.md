...
| 위치 | 상태 | 설명 |
|------|------|------|
| [20260803/260803-2.skill-portfolio-upstream-refresh-impl-maintenance.md](./20260803/260803-2.skill-portfolio-upstream-refresh-impl-maintenance.md) | Track A Phase 9 완료 / Track B Phase B0 완료 / Phase B1 (G9) 승인 대기 | `harness-kit` `0.3.1` 후보의 Track A는 `main`에 push됐고 `not-release-ready` 수동 gate를 유지한다. Track B는 read-only 사례 topology, Claude·Codex 공식 hook capability·bypass 경계, reuse/extend/new 분류와 실패 fixture를 B0 baseline으로 고정했다. 다음 B1은 portable project-owned bundle schema/template을 확정하는 단계이며 G9 승인 전에는 작성하지 않는다. |
