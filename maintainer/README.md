...
관리자는 별도 관리자 플러그인을 설치하지 않는다. 이 저장소에서는 repo-local 관리자 projection으로 유지보수 작업을 수행하고, 사용자와 같은 설치 인터페이스를 검증할 때만 `harness-kit` 사용자 플러그인을 별도 격리 설정에 설치한다. 관리자 projection과 사용자 플러그인 cache를 같은 디렉터리에 합치지 않는다.
