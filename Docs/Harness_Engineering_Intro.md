...
별도 관리자 플러그인은 만들지 않는다. 관리자는 이 저장소의 repo-local 관리자
스킬을 사용하고, 사용자 경험을 확인할 때만 일반 사용자용 플러그인을 격리 설치한다.
...
10. 내용 fingerprint와 완료 상태를 `.docs/.harness/humanize-handoffs.json`에 기록한다.
11. 승인 반영 후 원 producer의 구조 재검증을 통과한 최종 Markdown으로 구현·검증을 진행한다.
```
...
- 최종 Markdown 경로·내용 hash 기반 fingerprint를 기록해 새 session에서도 같은
  내용의 개선안을 다시 제안하지 않는다.
