...
- 릴리스 상태: Phase 7 기준 `not release-ready` — Codex/Claude 설치 표면의 실제 증적이 더 필요함
...
- CLI: marketplace source를 추가한 뒤 `codex plugin add ai-agent-harness` 계열 명령으로 설치한다.
- Codex Desktop/App: Plugins UI에서 Git-backed marketplace 또는 local plugin을 추가하고 새 task/session에서 스킬 노출을 확인한다.
- IDE extension: Phase 8 기준 별도 플러그인 설치 표면으로 보지 않는다. Codex CLI/App 플러그인 설치를 우선한다.

### Claude

- Claude Code CLI: plugin marketplace 추가, install/list/update/uninstall, `/reload-plugins` 순서로 확인한다.
...
Phase 7에서 이 호스트에서는 Codex CLI가 WindowsApps 권한 문제로 실행되지 않았고 Claude CLI는 설치되어 있지 않았다. 따라서 문서의 명령은 릴리스 전 수동/실환경 검증 항목과 함께 유지한다.
...
| 실제 프로젝트 | 플러그인 설치 후 `harness-setup`으로 `.docs`, `AGENTS.md`, `CLAUDE.md` 생성·갱신 | 프로젝트 수행자 |
